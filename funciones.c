#include <stdio.h>
#include <stdlib.h> //útil para la gestión de memoria
#include <string.h>
...
void ingresarProducto()
{
    ...
    // stdin es el flujo de entrada estándar y se utiliza como argumento para fgets
...
void editarProducto()
{
    ...
    // se lee cada linea del archivo productos.txt y se almacena en la variable linea, hasta que alcance el final del archivo 
    {
        if (contador == indice)
        {
            sscanf(linea, "%[^,],%d,%f", producto.nombre, &producto.cantidad, &producto.precio); // lee datos toma una cadena como entrada
